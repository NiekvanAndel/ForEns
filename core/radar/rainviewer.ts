...
export class RainViewerProvider implements RadarProvider {
  ...
  async nowcastProfile(lat: number, lon: number, signal?: AbortSignal): Promise<NowcastProfile> {
    // Nine quarter-hours covers now through +2h inclusive.
    const url =
...
/**
 * Turn a quarter-hourly precipitation series into the hero's bars.
 *
 ...
 */
export function buildProfile(data: MinutelyResponse): NowcastProfile {
  ...
  const bars: NowcastBar[] = BAR_OFFSETS.map((offsetMin) => {
    const mmPerHour = mmPerHourAt(offsetMin);
    return {
      offsetMin,
      mmPerHour,
  ...
  // Total over the window: each sample is already mm per quarter-hour.
  const totalMm = values.slice(0, 9).reduce((a, b) => a + b, 0);

  const firstWet = values.findIndex((v) => v > 0.05);
