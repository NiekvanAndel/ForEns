...
/**
 * The station behind a location, without asking it for anything.
 *
 * `useStationObservations` resolves this on its way to a fetch; the pages that only
 * need to know *whether* there is a station, and which, would otherwise have to
 * start a request to find out.
 */
export function useLocationStation(location: SavedLocation) {
  const { prefs } = usePrefs();
  const { data: stations } = useAgroStations();
  const useForCurrent = agroIntegration(prefs).useForCurrentLocation;

  return useMemo(
    () => stationForLocation(location, stations, useForCurrent),
    [location, stations, useForCurrent]
  );
}
